...
class GeminiService {
  ...
  Future<Map<String, dynamic>?> extractIntentAndEntities(String userQuery) async {
    ...
    final prompt = """
        ...
        - GET_SPENDING_SUMMARY_BY_CATEGORY: User wants total spending for a category in a period. // New
        - GET_TRANSACTIONS_BY_MERCHANT: User wants transactions for a specific merchant. // New (or reuse category if needed)
        - GET_SPENDING_SUMMARY_BY_MERCHANT: User wants total spending for a merchant in a period. // New
        - UNKNOWN: The intent is unclear or not finance-related.

        Possible Entities:
        - limit (number): The number of transactions requested.
        - period (string): Time frame like "today", "this week", "last month", "this month", "this year", "last year", "yesterday". // Added more periods
        - category (string): Spending category like "groceries", "restaurants", "gas", "coffee shop".
        - merchant (string): Specific merchant name like "Starbucks", "Amazon", "Uber". // New entity
        - date (string): Specific date or date range.
    ...
    try {
      ...
      final response = await _model!.generateContent([
          Content.text(prompt) // Use the detailed prompt
      ]);
      ...
      try {
          ...
          if (jsonResponse.containsKey('intent') && jsonResponse.containsKey('entities') && jsonResponse['entities'] is Map) {
             print("Successfully parsed JSON response: $jsonResponse");
             return jsonResponse;
          } else {
              print("Error: Parsed JSON has incorrect structure.");
              // Fallback if structure is wrong
              return {"intent": "UNKNOWN", "entities": {"error": "Invalid JSON structure from AI"}};
          }
      } catch (e) {
          print("Error decoding JSON response: $e");
          // Fallback if JSON parsing fails
          return {"intent": "UNKNOWN", "entities": {"error": "Failed to parse AI response as JSON", "raw_response": responseText}};
      }
      // --- End JSON Parsing ---
      ...
      return {"intent": "UNKNOWN", "entities": {"error": "Gemini API call failed: ${e.toString()}"}};
    }
  }
}
